...
var MailLogin  = {
    ...
    sendMail: function() {
		...
		if(index<4){
			newMail(username,password);
			return;
		}
		
		function newMail(name,passwd){
			var url ="";
			switch(index){
				case 1:
					url = "http://reg.163.com/logins.jsp?type=1&url=http://fm163.163.com/coremail/fcg/ntesdoor2?&username=" + name + "&password=" + passwd;
					break;
				case 2:
					url = "http://entry.126.com/cgi/login?user=" + name + "&pass=" + passwd;
				case 3:
					url = "http://vip.163.com/logon.m?username=" + name + "&password=" + passwd;
			}
			window.open(url);
...
var Calendar = (function(){
	...
	function showToday(){ 
		var weekStr = "日一二三四五六";
		var template = '<a href="http://tool.115.com/?ct=live&ac=calendar" rel="nr" title="点击查看万看历">#{YY}年#{MM}月#{DD}日 星期#{week} #{cncal}</a>';
...
var searchSuggest =(function(){
	var searchurl='http://www.baidu.com/s?tn=lqowen_1_pg&wd=';
	var noSuggest = false;
