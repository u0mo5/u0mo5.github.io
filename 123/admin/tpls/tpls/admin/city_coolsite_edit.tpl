    ...
    <div class="container">
        <div id="main">
            <div class="con box-green">
                <form action="?c=city_coolsite&a=save" method="post">
                ...
                <div class="box-content">
                    <table class="table-font">
                        ...
                        <tr>
                            <th style="vertical-align:top;">城市分类：</th>
                            <td>                               
                                <select name="cityclass_id">
                                    <option value='0'>选择……</option>
                                    <{foreach from=$cityclass_list item = i}>
                                        <option value="<{$i.id}>"<{if $data.cityclass_id eq $i.id}> selected="selected"<{/if}>><{$i.name}></option>
                                    <{/foreach}>
                                </select>
                            </td>
                        </tr>                        
                        <tr>
                            <th  style="vertical-align:top;">酷站分类：</th>
                            <td>                               
                                <select name="coolclass_id">
                                    <option value='0'>选择……</option>
                                    <{foreach from=$coolclass_list item = i}>
                                        <option value="<{$i.id}>"<{if $data.coolclass_id eq $i.id}> selected="selected"<{/if}>><{$i.name}></option>
                                    <{/foreach}>
                                </select>
                            </td>
                        </tr>    
                ...
                <div class="box-footer">
                    <div class="box-footer-inner">
                    	<input type="submit" value="提交" /></a> 或 <a href="?c=city_coolsite&a=index">取消</a>
                    </div>
                </div>
                </form>
            </div><!--/ con-->
